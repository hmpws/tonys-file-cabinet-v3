...
export function meta({ data }: Route.MetaArgs) {
  ...
  const fullTitle = date
    ? `${collectionName} - ${date} - ${title}`
  ...
  return [
    ...
    {
      name: "description",
      content: data.doc.article?.subtitle || "View document details",
    },
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const { collection, dbName } = await getCollectionConnection(
    client,
    params.name,
  );
  ...
  const statusMap: Record<
    string,
    { read: boolean; liked: boolean; tags: string[] }
  > = {};
  const statuses = await annotationDb
    .collection("#annotations")
    .find({
      documentId: { $in: allIds },
      range: null,
    })
    .project({ documentId: 1, read: 1, liked: 1, tags: 1 })
    .toArray();

  statuses.forEach((s) => {
    statusMap[s.documentId] = {
      read: !!s.read,
      liked: !!s.liked,
      tags: s.tags || [],
    };
  ...
  return {
    ...
    docStatus: statusMap[params.id as string] || {
      read: false,
      liked: false,
      tags: [],
    },
...
const ArticleContent = memo(
  ({ html, isDarkMode }: { html: string; isDarkMode: boolean }) => (
    <div
      id="article-content"
...
const Comment = ({
  ...
  <div className="group">
    ...
    <div
      className={`leading-relaxed text-base p-4 rounded-lg ${isDarkMode ? "bg-gray-800 text-gray-300" : "bg-gray-50 text-gray-700"}`}
    >
      {comment.body}
    </div>
    {comment.children && comment.children.length > 0 && (
      <div
        className={`ml-6 mt-4 space-y-4 pl-4 border-l-2 ${isDarkMode ? "border-gray-700" : "border-gray-100"}`}
      >
...
const MediaLink = ({
  ...
  return (
    <div className="flex items-center gap-2 max-w-full">
      <span
        className={`font-medium w-16 shrink-0 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
      >
        {label}:
      </span>
      <button
      ...
      >
        ...
        <span
          className={`transition-opacity shrink-0 opacity-0 group-hover:opacity-100 ${isDarkMode ? "text-gray-400" : "text-gray-400"}`}
        >
          <svg
...
export default function DocumentRoute({ loaderData }: Route.ComponentProps) {
  ...
  useEffect(() => {
    ...
    if (doc._id) {
      annotationFetcher.load(
        `/api/annotations?documentId=${doc._id}&collectionName=${collectionName}`,
      );
    }
  ...
  useEffect(() => {
    ...
    if (relevantAnnotations.length > 0) {
      relevantAnnotations.forEach((ann) => {
        try {
          ...
          let mark = articleRef.current?.querySelector(
            `mark[data-annotation-id="${ann._id}"]`,
          ) as HTMLElement;
          ...
          if (mark) {
            ...
            while (
              parent &&
              parent !== articleRef.current &&
              articleRef.current?.contains(parent) &&
              limit < 50
            ) {
    ...
    const generalAnnotation = relevantAnnotations.find(
      (a) => !a.range || a.range === "null",
    );
    ...
    const labeledPositions = newPositions.map((p) => {
      let index = 0;
      if (p.range && !p.isGeneral) {
        // Only increment for actual highlights
        index = currentIndex++;
        if (p._id.startsWith("temp-") === false) {
          const mark = articleRef.current?.querySelector(
            `mark[data-annotation-id="${p._id}"]`,
          ) as HTMLElement;
          if (mark) {
  ...
  useLayoutEffect(() => {
    ...
    const nodes = Array.from(
      document.querySelectorAll(".side-note-card"),
    ) as HTMLElement[];
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      const { activeAnnotation, editingComment, editingTags } =
  ...
  const performSave = useCallback(
    (id: string, comment: string, tags: string[]) => {
      const isGeneral =
        id === "general-placeholder" || id.startsWith("temp-general");
  ...
  const lastDocElementRef = useCallback(
        ...
        if (entries[0].isIntersecting) {
          ...
          debounceRef.current = setTimeout(async () => {
            ...
            if (page < totalPages && currentlyOnline) {
              ...
              fetcher.load(
                `?page=${nextPage}${searchTerm ? `&q=${searchTerm}` : ""}`,
              );
              setPage(nextPage);
            }
          }, 300);
  ...
  const handleSave = () => {
    if (
      activeAnnotation?.id.startsWith("temp-") &&
      activeAnnotation.id !== "general-placeholder" &&
      !activeAnnotation.id.startsWith("temp-general")
    ) {
  ...
  const deleteAnnotation = () => {
    ...
    const highlights = articleRef.current?.querySelectorAll(
      `mark[data-annotation-id="${activeAnnotation.id}"]`,
    );
  ...
  const schemaData = {
    ...
    datePublished: doc.article?.post_date
      ? new Date(doc.article.post_date).toISOString()
      : undefined,
  ...
  return (
    <div className="min-h-screen bg-white font-sans flex flex-col md:flex-row relative">
            ...
            {docs.map((d, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      className={`block font-medium text-sm flex items-center gap-2 ${d.id === doc._id ? "font-bold" : ""}`}
                    >
                      <AudienceIcon audience={(d as any).audience} />
                      <span className="truncate flex-1">{d.title}</span>
                      <span className="flex items-center gap-1 shrink-0">
                        {(d as any).read && (
                          <span title="Read" className="text-xs">
                            ✅
                          </span>
                        )}
                        {(d as any).liked && (
                          <span title="Liked" className="text-xs">
                            ❤️
                          </span>
                        )}
                      </span>
                      ...
                      {(d as any).tags && (d as any).tags.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {(d as any).tags.map((tag: string, i: number) => (
                            <span
                              key={i}
                              className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-gray-600 border border-gray-200"
                            >
                              {tag}
                            </span>
                          ))}
      ...
      {/* Main Content */}
      <div
        className={`flex-1 order-1 md:order-2 min-h-screen p-4 flex flex-col items-center print:overflow-visible transition-colors duration-300 ${isSidebarOpen ? "opacity-50 pointer-events-none" : ""} ${isDarkMode ? "bg-gray-900" : "bg-white"} print:bg-white`}
      >
        <div className="max-w-[1600px] w-full transition-all duration-300 print:max-w-none print:w-full">
          <main className="px-6 pb-20 print:pb-0">
            ...
            <div
              ref={articleRef}
              className="relative group mx-auto print:mx-0 lg:grid lg:grid-cols-[minmax(0,1fr)_300px] 2xl:grid-cols-[minmax(0,1fr)_800px_minmax(0,1fr)] lg:gap-12 print:grid print:grid-cols-[minmax(0,1fr)_200px] print:gap-6 max-w-[1150px] md:max-w-[800px] lg:max-w-[1150px] 2xl:max-w-none print:max-w-none"
            >
              {/* Content Column */}
              <div className="lg:min-w-0 2xl:col-start-2">
                ...
                <header className="pt-12 pb-8 relative">
                  ...
                  <h1
                    ref={titleRef}
                    className={`text-4xl font-bold leading-tight mb-2 ${isDarkMode ? "text-gray-100" : "text-gray-900"} print:text-black`}
                  >
                    {doc.article?.title || "Untitled Document"}
                  </h1>
                  {doc.article?.subtitle && (
                    <h2
                      className={`text-xl font-serif leading-relaxed mb-4 ${isDarkMode ? "text-gray-400" : "text-gray-500"} print:text-gray-500`}
                    >
                      {doc.article.subtitle}
                    </h2>
                  )}
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-4">
                    {docStatus.tags &&
                      docStatus.tags.map((tag: string, i: number) => (
                        <span
                          key={i}
                          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${isDarkMode ? "bg-gray-800 text-gray-300 border-gray-600 hover:bg-gray-700" : "bg-gray-100 text-gray-700 hover:bg-gray-200 border-gray-200"}`}
                        >
                          {tag}
                          <button
                    ...
                    <div>
                      {doc.article?.post_date && (
                        <time dateTime={doc.article.post_date}>
                          {new Date(doc.article.post_date).toLocaleDateString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "long",
                          ...
                          {docStatus.read ? (
                            ...
                            >
                              <path
                                ...
                                opacity="0"
                              />{" "}
                              {/* Hidden check for sizing/visual consistency if needed, or just a box */}
                              <rect
                                x="3"
                                y="3"
                                width="18"
                  ...
                  {(doc.pdf ||
                    ...
                    (doc.media && doc.media.length > 0)) && (
                    <div
                      className={`mt-6 p-4 rounded-lg border ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-gray-50 border-gray-100"}`}
                    >
                      <h3
                        className={`text-sm font-bold uppercase tracking-wide mb-3 ${isDarkMode ? "text-gray-200" : "text-gray-900"}`}
                      >
                        Media Files
                      </h3>
                ...
                {doc.transcript && (
                  <section className="mt-16 border-t border-gray-100 pt-10">
                    ...
                    <div
                      className={`prose prose-lg leading-relaxed p-6 rounded-xl border whitespace-pre-wrap font-serif w-full max-w-none ${isDarkMode ? "bg-gray-800 border-gray-700 prose-invert text-gray-300" : "bg-gray-50 border-gray-100 prose-slate text-gray-700"}`}
                    >
                      {doc.transcript}
                    </div>
                  </section>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(ann.isGeneral || !ann.range) &&
                          /* Nothing here now */ null}
                        {ann.comment ? (
                          <div className="text-gray-800 min-w-0 break-words">
                            {ann.comment}
                          </div>
                        ) : (
          ...
          {selection && (
            ...
            >
              <button
                onClick={() => saveAnnotation("#fef9c3")}
                className="w-6 h-6 rounded-full bg-[#fef9c3] hover:scale-110 transition-transform border border-gray-200"
                title="Yellow"
              ></button>
              <button
                onClick={() => saveAnnotation("#dcfce7")}
                className="w-6 h-6 rounded-full bg-[#dcfce7] hover:scale-110 transition-transform border border-gray-200"
                title="Green"
              ></button>
              <button
                onClick={() => saveAnnotation("#fce7f3")}
                className="w-6 h-6 rounded-full bg-[#fce7f3] hover:scale-110 transition-transform border border-gray-200"
                title="Pink"
              ></button>
            </div>
          )}
          ...
          {activeAnnotation &&
            (() => {
              const currentAnn = annotations.find(
                (a) => a._id === activeAnnotation.id,
              );
              const isGeneral =
                activeAnnotation.id === "general-placeholder" ||
                (currentAnn &&
                  (!currentAnn.range || (currentAnn as any).isGeneral));
